...
class BannedChannels(interactions.Extension):
    ...
    async def select(self, ctx: interactions.ComponentContext, choice: list[int]):
        guild = await ctx.get_guild()
        conn = sqlite3.connect(f"./Database/{guild.id}.db")
        c = conn.cursor()

        for i in range(len(choice)):
            c.execute(f"""SELECT hidden from channels WHERE id = {choice[i]}""")
            row = c.fetchone()
            if row is None:
                c.execute(f"""INSERT INTO channels VALUES (name ,{choice[i]}, NULL, 1)""") # N'a aucun sens (faut que je la modifie)
                channel = await interactions.get(self.bot, interactions.Channel, object_id=choice[i])
